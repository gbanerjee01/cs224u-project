...
#     jname = "./CTC_LM_rescore_splits/test.json"
...
#     jname = "./CTC_LM_rescore_splits/dev.json"
#     with open(jname, "w") as outfile: 
#         json.dump(pred_dict, outfile)
    ...
    for i in tqdm(range(2000, len(filelist))):
        filename = filelist[i]
        path = '../cv-corpus-6.1-2020-12-11/en/clips/' + filename
        ...
        if i%1000==0:
            ...
            with open(jname, "w") as outfile: 
                json.dump(pred_dict, outfile)
    jname = "./CTC_LM_rescore_splits/train2.json"
    with open(jname, "w") as outfile: 
        json.dump(pred_dict, outfile)
